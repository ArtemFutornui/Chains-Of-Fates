...
func create_chunk_image() -> void:
	map_images.clear()
	var prev_res: int = mapGenerator.map_texture_resolution_per_chunk
	mapGenerator.map_texture_resolution_per_chunk = 64
	var batch := 8
	var processed := 0
	while chunks_coords.size() != 0:
		if is_terminated():
			mapGenerator.map_texture_resolution_per_chunk = prev_res
			return
		...
		processed += 1
		if processed % batch == 0 or chunks_coords.size() == 0:
