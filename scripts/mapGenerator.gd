...
var forest_max_count:int = 22
var forest_min_radius:float = 350.0
var forest_max_radius:float = 700.0
var trees_min_spacing:float = 6.5
var trees_max_spacing:float = 9.0
var trees_random_offset_spacing:float = 3.0

var resolution_reduction_threshold:int = 3
var layers_from_border: int = 3 

var map_texture_resolution_per_chunk := 32
...
func generate_chunk(x:int, z: int) -> Node:
	...
	var chunk_mat: ShaderMaterial = _terrain_material.duplicate()
	terrain.material_override = chunk_mat
	...
	for obj in _objectsList:
		...
		if obj.object_name == "Deposit":
			...
			if _river_chunks.has(Vector2(x, z)) or _lake_chunks.has(Vector2(x, z)):
				count_objects = int(count_objects * 2.2)
			var to_exit: int = 0
...
func _smooth_points_avg(points: Array[Vector2], iterations: int) -> Array[Vector2]:
	...
	for k in range(iterations):
		...
		for i in range(1, result.size() - 1):
			var p0: Vector2 = result[i - 1]
			var p1: Vector2 = result[i]
			var p2: Vector2 = result[i + 1]
			var avg: Vector2 = (p0 + p1 + p2) / 3.0
			out.append(avg)
		out.append(result.back())
		result = out
	return result
...
world_pos: Vector2,tx:float,tz:float, object_heights: Dictionary[Array,float], multimesh_height:float) -> Dictionary:
	...
	if dist_to_centr < 20.0:
		return {}
	if dist_to_centr < center_radius:
		var chance = pow(smoothstep(0.0, center_radius, dist_to_centr), 1.3)
...
func _add_forest_decorations(pos : Vector2, chunk_pos: Vector2) -> Array:
	...
	for dec in _forest_decorations:
		...
		if dec_data.size() == 0:
			continue
		# Заборонити перетин грибів з родовищами
		if dec.object_name == "mushrooms":
			var m_size: float = dec_data["size"]
			if !_is_far_from_all_deposits(dec_pos, m_size, 2.2, object_type):
				continue
		...
		if dec.object_name == "mushrooms":
			...
			var filtered_mushrooms := []
			for m in m_objects:
				var m_pos_local : Vector3 = m["position"]
				var m_world := Vector2(chunk_pos.x + m_pos_local.x, chunk_pos.y + m_pos_local.z)
				var m_size2: float = m["size"]
				if !_is_far_from_all_deposits(m_world, m_size2, 2.2, object_type):
					continue
				filtered_mushrooms.append(m)
			decors.append_array(filtered_mushrooms)
	return decors
...
func _get_local_river_width(p: Vector2) -> float:
	if _river_chunks.size() == 0:
		return 0.0
	var c: Vector2 = _get_chunk_position_from_point(p)
	if not _river_chunks.has(c):
		return 0.0
	var min_p: int = _river_chunks[c].min() - 5
	var max_p: int = _river_chunks[c].max() + 5
	var start_index: int = max(min_p, 0)
	var end_index: int = min(max_p, _river_data.size()-1)
	var points_group := _river_data.slice(start_index,end_index+1)
	for i in range(points_group.size()-1):
		var a: Vector2 = points_group[i].pos
		var b: Vector2 = points_group[i+1].pos
		var ab := b - a
		var ap := p - a
		var t: float = clamp(ap.dot(ab) / ab.length_squared(), 0.0, 1.0)
		var closest: Vector2 = a + ab * t
		var dist: float = p.distance_to(closest)
		var width: float = lerp(points_group[i].width, points_group[i+1].width, 0.5)
		if dist < width:
			return width
	return 0.0

func _is_in_lake_area(p: Vector2) -> bool:
	if _lake_chunks.size() == 0:
		return false
	var c: Vector2 = _get_chunk_position_from_point(p)
	if not _lake_chunks.has(c):
		return false
	for lake_point in _lake_data:
		var pos: Vector2 = lake_point.pos
		var width: float = lake_point.width
		if p.distance_to(pos) < width:
			return true
	return false
